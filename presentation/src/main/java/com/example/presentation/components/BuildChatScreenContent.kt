            ...
            MessageRow(messages, authViewModel) {
                ...
                SubcomposeAsyncImage(
                    ...
                    modifier = Modifier
                        .fillMaxWidth()
                        .clickable {
                            messages.messageText?.let {messageText ->
                                extractLatLongFromUrl(messageText)?.let { (lat,long) -> openGoogleMaps(context, latitude = long, longitude = lat) }
                            }                        }
                )
            }
        }
...
private fun ShowVoiceDuration(
    ...
    applicationViewModel: ApplicationViewModel
        ...
        if (hasPermissions(applicationViewModel.application, storagePermissions)) {
            val filePath = "$root/Chat/Recorders/${messages.messageText}"
            val file = File(filePath)
            val duration = if (file.exists()) {
                getDuration(filePath, applicationViewModel.application, null)
            } else {
                getDuration(filePath, applicationViewModel.application, messages.messageFile)
            }
            Text(text = duration.toString())
        } else {
            val filePath = "$root/Chat/Recorders/${messages.messageText}"
            val duration = getDuration(filePath, applicationViewModel.application, messages.messageFile)
            Text(text = duration.toString())
        }
}
